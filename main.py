...
### 🔍 추출 및 필터링 규칙 (엄격 준수 - 위반 시 무효):
1. **오늘 날짜**: {datetime.now().strftime('%Y-%m-%d')}
2. **카테고리 분류**: '커뮤니티', '오프라인', '인 게임' 중 하나로 분류. 특히 **캐릭터/무기 모집(뽑기), 인게임 미니게임/퍼즐**은 무조건 **'인 게임'**으로 분류할 것.
3. **포함**: 기간 한정 이벤트, 웹 이벤트, 콜라보레이션, 캐릭터/무기 모집(튜닝). 명조의 **'이벤트 튜닝'** 소식은 필수 포함.
4. **제외**: 단순 접속/출석체크(로그인 보상), 상시/정규 이벤트, **이미 종료된 이벤트**.
5. **날짜 강제 추출 (핵심)**: '진행 중'이라는 표현은 절대 사용 금지. 본문에서 `일시`, `기간`, `점검 후`, `업데이트 후`, `~`, `/` 등의 키워드를 낱낱이 뒤져 **정확한 시작일과 종료일(시각 포함 가능)**을 찾아낼 것. 
   - 예: "3월 29일 04:00 ~ 4월 17일 03:59"
   - 날짜가 숫자 형태로 본문에 있다면 반드시 그 숫자를 그대로 가져올 것. '정보 없음'이나 '상시'는 정말 본문에 날짜가 없을 때만 극히 예외적으로 허용.
